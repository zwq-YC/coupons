    ...
    httpRequest.onreadystatechange = function () {
        if (httpRequest.readyState == 4 && httpRequest.status == 200) {
            ...
            if (data == "当日已领取优惠券" || data == "来晚了，券已领完") {
                var url = 'https://sctapi.ftqq.com/SCT146464T9GEUSrAYZmMZPGpTDPBYI4tu.send?title=xiaobaosuccess'
...
function quan1(){
    ...
    httpRequest.onreadystatechange = function () {
        if (httpRequest.readyState == 4 && httpRequest.status == 200) {
            ...
            console.log('大宝' + data)
            if (data == "当日已领取优惠券" || data == "来晚了，券已领完") {
                var url = 'https://sctapi.ftqq.com/SCT146464T9GEUSrAYZmMZPGpTDPBYI4tu.send?title=dabaosuccess'
                var http = new XMLHttpRequest();//第一步：建立所需的对象
                http.open('POST', url, true);//第二步：打开连接  将请求参数写在url中  ps:"http://localhost:8080/rest/xxx"
                http.send();//第三步：发送请求  将请求参数写在URL中
                clearInterval(j)
